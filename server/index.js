...
app.post("/createTeacher", (req, res) => {
  ...
  db.query(
    "INSERT INTO teacher (t_id, t_name, t_dept, t_mail, t_mobile, t_des) VALUES (?,?,?,?,?,?) ",
    [
      teacherID,
      teacherName,
      teacherDepartment,
...
app.post("/createStudent", (req, res) => {
  ...
  db.query(
    "INSERT INTO student (s_reg, s_name, s_roll, s_mail1, s_mail2, s_mobile1, s_mobile2, s_semester ) VALUES (?,?,?,?,?,?,?,?) ",
    [
      studentReg,
      studentName,
      studentRoll,
...
app.post("/createOfficer", (req, res) => {
  ...
  db.query(
    "INSERT INTO officer (o_id, o_name, o_des, o_mail, o_mobile) VALUES (?,?,?,?,?) ",
    [officerID, officerName, officerDesignation, officerMail, officerMobile],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/createCourse", (req, res) => {
  ...
  db.query(
    "INSERT INTO course (c_uid, c_name, c_code, c_credit) VALUES (?,?,?,?) ",
    [courseCode + "2022", courseName, courseCode, courseCredit],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/createSemester", (req, res) => {
  ...
  db.query(
    "INSERT INTO semester (semester_uid, semester_name, year, o_id) VALUES (?,?,?,?) ",
    [semesterName + semesterYear, semesterName, semesterYear, semesterOId],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
