...
import {SideBarData} from '../LocalData/sideBarData';
import './navBar.css';
        ...
        <nav className={sidebar ? 'nav-menu active fixed top-0 flex h-screen drop-shadow-2xl' : 'nav-menu flex fixed top-0 pt-5 h-screen'}>
            <ul className='nav-menu-items p-2 pt-5 text-xl text-left text-white bg-blue-500' onClick={showSideBar}>
                ...
                {SideBarData.map((item, index) => {
                    return (
                        <li key={index} className= 'mb-6 mt-4 pl-5 pr-20 block rounded-lg hover:bg-white hover:text-black active:bg-white'>
                            <Link to={item.path}>
                                <div className='flex py-3'>
                                    <span className='mr-4 pt-1'>{item.icon}</span>
